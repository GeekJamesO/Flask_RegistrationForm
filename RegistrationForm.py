...
def  process():
    ...
    if (len(email) < 1):
        ...
        validated = False
        flash("'{}' is an invalid Email Address!".format(email))
    else:
        print "Email is good"
    ...
    if (len(first_name) < 1):
        ...
        validated = False
        flash("'{}' numbers are not allowed in first name!".format(first_name))
    else:
    ...
    if (len(last_name) < 1):
        ...
        validated = False
        flash("'{}' numbers are not allowed in last name!".format(last_name))
    else:
        print "Last Name is good"

    if (len(password1) < 8):
        validated = False
        flash("First Password must not be atleast 8 characters!")

    if (len(password2) < 8):
        validated = False
        flash("Second Password must not be atleast 8 characters!")
